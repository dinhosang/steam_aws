...
_aws_resources_ec2_module() {
    ...
    create_ec2() {
        ...
        local -r created_instance_details=$(
        ...
        while [ $instance_running == false ]; do
            ...
            instance_details_found=$(
                aws --profile $AWS_PROFILE ec2 describe-instance-status \
                    --region $AWS_REGION \
...
if [[ ${AWS_RESOURCES_EC2_MODULE_IMPORTED:=false} == false ]]; then
